...
export async function GET() {
  try {
    ...
    const [members, contributions, projects, expenses, audit, activeMembers, monthlyContributions, activeProjects] = await Promise.all([
      prisma.tckMember.findMany({ select: { memberCode: true, name: true, phone: true, zone: true, country: true, status: true, createdAt: true }, orderBy: { createdAt: "desc" }, take: 250 }),
