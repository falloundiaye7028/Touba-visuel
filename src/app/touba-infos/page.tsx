    ...
    return (
      <>
        ...
        <section className="mx-auto grid max-w-[1400px] gap-8 px-4 pb-12 lg:grid-cols-[minmax(0,1fr)_330px]">
          <div>
            <SectionHeading titre="Dernières actualités" href="/touba-infos/fil-info" hrefLabel="Tout le fil" />
          ...
          <aside className="space-y-7">
            ...
            <div className="bg-neutral-950 p-6 text-white">
              ...
              <Link href="/touba-infos/newsletter" className="mt-5 inline-flex items-center gap-2 bg-green-600 px-4 py-3 text-sm font-black text-white hover:bg-green-700">
        ...
        <section className="border-y border-neutral-200 bg-neutral-50">
          <div className="mx-auto max-w-[1400px] px-4 py-12">
            <div className="grid gap-10 lg:grid-cols-3">
              <EditorialColumn title="Politique" articles={politique} href="/touba-infos/rubrique/politique" />
              <EditorialColumn title="Économie" articles={economie} href="/touba-infos/rubrique/economie" />
              <EditorialColumn title="Société" articles={societe} href="/touba-infos/rubrique/societe" />
            </div>
          </div>
        </section>
          ...
          <section className="bg-neutral-950 text-white">
            <div className="mx-auto max-w-[1400px] px-4 py-12">
              <div className="mb-6 flex items-end justify-between gap-4 border-b border-white/15 pb-3">
                ...
                <Link href="/touba-infos/videos" className="text-xs font-black uppercase tracking-wide text-green-400">
                  Toutes les vidéos
                </Link>
              </div>
              <div className="grid gap-6 md:grid-cols-3">
                {videos.map((article) => (
                  <Link key={article.id} href={`/touba-infos/${article.slug}`} className="group">
        ...
        <section className="mx-auto max-w-[1400px] px-4 py-12">
          ...
          <div className="flex flex-wrap gap-2">
            {[...new Set(articles.map((article) => article.categorie))].map((categorie) => (
              <Link
                key={categorie}
                href={`/touba-infos/rubrique/${slugCategorie(categorie)}`}
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1.55fr)_minmax(420px,1fr)]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {manchettes.map((article) => (
              <Link key={article.id} href={`/touba-infos/${article.slug}`} className="group border-b border-neutral-200 pb-4 sm:border-b-0 sm:pb-0">
...
function EmptyState() {
  return (
    <section className="mx-auto flex min-h-[55vh] max-w-3xl flex-col items-center justify-center px-4 py-16 text-center">
      ...
      <Link href="/touba-infos/contact#alerte" className="mt-7 inline-flex items-center gap-2 bg-green-600 px-5 py-3 text-sm font-bold text-white hover:bg-green-700">Contacter la rédaction <Send size={16} /></Link>
    </section>
  );
}
...
function ErrorState() {
  return (
    <section className="mx-auto flex min-h-[55vh] max-w-3xl flex-col items-center justify-center px-4 py-16 text-center">
      ...
      <Link href="/touba-infos/contact#alerte" className="mt-7 inline-flex items-center gap-2 bg-green-600 px-5 py-3 text-sm font-bold text-white hover:bg-green-700">Contacter la rédaction <Bell size={16} /></Link>
    </section>
  );
}
