...
export function AuthForm({ mode }: { mode: "login" | "register" | "forgot" }) {
  ...
  const submit=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();setLoading(true);setError("");const data=new FormData(event.currentTarget);const email=String(data.get("email"));const password=String(data.get("password")??"");
    try { if(mode==="forgot"){await new Promise((resolve)=>setTimeout(resolve,500));setSuccess("Si cette adresse existe, un lien de réinitialisation a été envoyé.");return}
      if(mode==="register"){trackPublicEvent("register_started");const response=await fetch("/api/register",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:String(data.get("name")),email,password})});if(!response.ok){const payload=await response.json() as {error?:string};throw new Error(payload.error??"Inscription impossible")}await signIn("credentials",{email,password,redirect:false});trackPublicEvent("register_completed");router.push("/onboarding");return}
